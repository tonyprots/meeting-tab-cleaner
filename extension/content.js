// Глаза расширения на наблюдаемой странице. Отвечает на три вопроса.
//
...
//
// 3. «Передала ли я действие приложению» (handedOff). Нужен только
//    записям реестра с `handoff: true` — страницам, где кнопка «Открыть
//    в приложении» соседствует с содержимым (пост в t.me). Признак — переход
//    по адресу с чужой схемой (`tg://…`). Navigation API сообщает о нём
//    событием `navigate`, хотя сама страница никуда не уходит: переход
//    перехватывает браузер и отдаёт приложению. Этот путь покрывает и клик
//    по ссылке, и `location.href = …` из скрипта страницы.
//
...
// Схемы, которые остаются в браузере. Всё прочее — tg:, zoommtg:,
// msteams: и так далее — открывает внешнее приложение.
const IN_BROWSER = /^(https?|about|blob|data|javascript|chrome|chrome-extension|file):/i;
let handedOff = false;

globalThis.navigation?.addEventListener("navigate", (event) => {
  if (handedOff || IN_BROWSER.test(event.destination?.url ?? "https:")) return;
  handedOff = true;
  report();
});
